...
router.post('/', (req, res) => {
  const db = getDB()
  const { ship_id, berth_time, expected_power, expected_duration, applicant, applicant_phone, remark } = req.body
  ...
  const info = db.prepare(`INSERT INTO applications (app_no, ship_id, berth_time, expected_power, expected_duration, applicant, applicant_phone, remark)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run([appNo, ship_id, berth_time, expected_power || null, expected_duration || null, applicant || null, applicant_phone || null, remark || null])
...
router.put('/:id', (req, res) => {
  ...
  const { berth_time, expected_power, expected_duration, applicant, applicant_phone, remark } = req.body
  db.prepare(`UPDATE applications SET berth_time=?, expected_power=?, expected_duration=?, applicant=?, applicant_phone=?, remark=?, updated_at=datetime('now','localtime')
    WHERE id=?`).run([berth_time, expected_power || null, expected_duration || null, applicant || null, applicant_phone || null, remark || null, req.params.id])
